...
TOTAL_COUNT=$(wc -l alpha 2025-08-02_emerg_data_organized_step_four.csv)
printf "total records: %s\n" "$TOTAL_COUNT" 

for abcde_subtype in ${ABCDE_SUB_TYPES[@]}; do
	printf "%s records: of %s type\n" "$abcde_subtype" "$(grep -i $abcde_subtype ./2025-08-02_emerg_data_organized_step_four.csv | wc -l)"
